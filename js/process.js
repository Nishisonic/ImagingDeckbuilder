        ...
        events: {
            on: {
                animation: {
                    ...
                    open: function () {
                        // 装備選択レイアウト作成
                        $( "#sidebar" ).append( '<div style="color:#1FC1C3; padding-top:5px; padding-bottom:50px; text-align:center;"><nav style="position:fixed;">装備選択</nav></div>' );
                        for ( let id in ITEM_TYPE3_DATA ) {
                            $( "#sidebar" ).append( '<div style="margin:3px; border:1px solid black; text-align:center; color:#FFF; cursor:pointer;">' + ITEM_TYPE3_DATA[ id ] + '</div>' );
                        }
                        $( "#sidebar" ).append( '<div style="margin:3px; border:1px solid black; text-align:center; color:#FFF; cursor:pointer;">装備を外す</div>' );
                    },
                    close: function () {
                        $( "#sidebar" ).empty();
                    }
